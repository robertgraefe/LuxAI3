...
try:
    from agentV2_utils.actions import DIRECTION
except Exception:
...
class Tile:
    ...
    def value(self) -> float:
        if self.type == TILETYPE.ASTEROID:
            return 0
        if self.type == TILETYPE.NEBOLA:
            return 0
        if self.type == TILETYPE.UNKOWN:
            return 0.05
        if self.is_relic:
            return 1
        return 0.01
...
class Observation:
    def __init__(self, player_team_id: int, opponent_team_id: int, obs: dict[str, any], actions: np.ndarray,
                 last_observation: 'Observation' = None):
        # Players
        self.player = Player()
        self.opponent = Player()

        self.player.points = obs["team_points"][player_team_id]
        self.opponent.points = obs["team_points"][opponent_team_id]
        self.player.wins = obs["team_wins"][player_team_id]
        self.opponent.wins = obs["team_wins"][opponent_team_id]
